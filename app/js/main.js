$(document).ready(function() {
	...
	var headerDefaultBackgroundColor = '#666666';
	var headerDefaultFontColor = '#F9F9F9';
	...
	function updateHeaderWithSection(section){
		...
		var headerBackgroundColor = headerDefaultBackgroundColor
        var headerFontColor = headerDefaultFontColor

		var previousSection = section - 1;
		if(previousSection >= 0){
			headerBackgroundColor = sectionBackgroundColors[previousSection]
        	headerFontColor = sectionFontColors[previousSection]
		}

		var header = $('#header')
		header.css("background-color", headerBackgroundColor)
		header.css("color",headerFontColor)
	}
	...
	function updateLayout(){
		...
		if(windowWidth < windowHeight){
			isPortrait = true
			minWindowSize = windowWidth
		}
		$('.leftBox').toggleClass('forceFullWidth',isPortrait) //Portrait -> Full width

		//Zoom
		var needsToZoom = false
		if(isMobile && isPortrait){
			needsToZoom = true
		}
		var resFactor = (needsToZoom)? 1.2 : 1
